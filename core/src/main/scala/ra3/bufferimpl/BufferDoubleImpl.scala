...
private[ra3] trait BufferDoubleImpl { self: BufferDouble =>
  ...
  def elementwise_lteq(other: Double): BufferInt = {
    ...
    while (i < n) {
      r(i) = if (isMissing(i) || other.isNaN) BufferInt.MissingValue else if (self.values(i) <= other) 1 else 0
      i += 1
    }
    BufferInt(r)
  }
  ...
  def elementwise_containedIn(other: Set[Double]): BufferInt = {
    ...
    while (i < n) {
      r(i) = if (isMissing(i)) BufferInt.MissingValue else if (other.contains(self.values(i))) 1 else 0
      i += 1
    }
    BufferInt(r)
  }
