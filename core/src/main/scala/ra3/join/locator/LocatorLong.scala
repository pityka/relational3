...
private[ra3] class LocatorLong(val allKeys: Array[Long]) {
  private val (cts, uniqueIdx) = ra3.hashtable.LongTable
  ...
  private val map = ra3.hashtable.LongTable
    .build(allKeys, null)

  def contains(key: Long): Boolean = map.contains(key)
  def get(key: Long): Int = map.lookupIdx(key)
  def getAll(key: Long): Array[Int] = if (map.contains(key))
    map.lookupAllIdx(key)
  else Locator.emptyArray

  def count(key: Long): Int = {
