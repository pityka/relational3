...
/** We often need to "reindex" one array/vec/series/frame so as to produce a new
  * one.
  *
  * First, note that an array A of values of type T is actually a mapping
  * defined as
  *
  * {{{
  *                 f
  ...
  * de-referenced.
  *
  * ReIndexer has two fields, lTake and rTake, which are array-based maps
  * defined for some integers M and N as
  *
  * {{{
  *           g
  *   [0, N) => [-1, M)
  ...
  * }}}
  *
  * For this to work, the maximum value M in the co-domain of g must be <=
  * A.length. We also augment the mapping f to send -1 to the corresponding NA
  * value of type T.
  *
  * For example, suppose we have Index(0,1,2,4) and Index(0,1,2,3). Performing a
  * full outer join would yield the following ReIndexer:
  *
  *   - lTake = {0, 1, 2, -1, 3},
  *   - rTake = {0, 1, 2, 3, -1},
  ...
  * where we make a value of None behave as if it were an array of [0, N). So,
  * the "taking" code knows to take the original values. You will therefore see
...
object ReIndexer {
  ...
  /** Factory to produce a new [[org.saddle.index.ReIndexer]] instance
    ...
    */
  def apply[T](lft: Option[Array[Int]], rgt: Option[Array[Int]]) =
    new ReIndexer[T] {
      def lTake = lft
      def rTake = rgt
    }
