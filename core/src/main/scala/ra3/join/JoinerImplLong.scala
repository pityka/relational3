...
private[ra3] object JoinerImplLong {
  ...
  private def leftJoin(left: LocatorLong, right: LocatorLong): ReIndexer = {
    val ll = left.length
    ...
    while (i < ll) {
      ...
      if (ra3.BufferLong.MissingValue == otherVal) {
        rightBuffer.+=(-1)
        leftBuffer.+=(i)
      } else {
  ...
  def innerJoin(left: LocatorLong, right: LocatorLong): ReIndexer = {
    ...
    while (i < ltmp.length) {
      val k = ltmpKeys(i)
      if (k == ra3.BufferLong.MissingValue) {
        ()
      } else {
        val c = rtmp.count(k)
        if (c == 0) {
  ...
  def outerJoin(left: LocatorLong, right: LocatorLong): ReIndexer = {
    // hits hashmap
    ...
    while (i < left.length) {
      val v = leftKeys(i)
      if (v == ra3.BufferLong.MissingValue) {
        lft.+=(i)
        rgt.+=(-1)
      } else {
        val c = right.count(v)
