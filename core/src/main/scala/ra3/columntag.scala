...
sealed trait ColumnTag { self =>
  ...
  def countNonMissing(column: ColumnType)(implicit
      tsc: TaskSystemComponents
  ): IO[Long] =
    ra3.ts.CountNonMissing.queue(this)(segments(column))
...
object ColumnTag {
  object I32 extends ColumnTag {
    ...
    ): ra3.TaggedColumn = TaggedColumn.TaggedColumnI32(first)
    ...
    def cdf(buffer: BufferType, numPoints: Int): (BufferInt, BufferDouble) = {
      ...
      val sorted =
        org.saddle.array.sort[Int](org.saddle.Vec(buffer.values).dropNA.toArray)
  ...
  object I64 extends ColumnTag {
    ...
    def makeTaggedColumn(
        first: ra3.ColumnTag.I64.ColumnType
    ): ra3.TaggedColumn = TaggedColumn.TaggedColumnI64(first)
    def makeTaggedSegment(
    ...
    def cdf(buffer: BufferType, numPoints: Int): (BufferLong, BufferDouble) = {
      ...
      val sorted = org.saddle.array
        .sort[Long](org.saddle.Vec(buffer.values).dropNA.toArray)
      val cdf = percentiles.map { p =>
        val idx = (p * (sorted.length - 1)).toInt
        (sorted(idx), p)
      }
  ...
  object Instant extends ColumnTag {
    ...
    def makeTaggedColumn(
        first: ra3.ColumnTag.Instant.ColumnType
    ): ra3.TaggedColumn = TaggedColumn.TaggedColumnInstant(first)
    def makeTaggedSegment(
  ...
  object StringTag extends ColumnTag {
    ...
    def makeTaggedColumn(
        first: ra3.ColumnTag.StringTag.ColumnType
    ): ra3.TaggedColumn = TaggedColumn.TaggedColumnString(first)
    def makeTaggedSegment(
  ...
  object F64 extends ColumnTag {
    ...
    def makeTaggedColumn(
        first: ra3.ColumnTag.F64.ColumnType
    ): ra3.TaggedColumn = TaggedColumn.TaggedColumnF64(first)
    def makeTaggedSegment(
