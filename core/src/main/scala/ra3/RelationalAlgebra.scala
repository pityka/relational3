    ...
    name.flatMap { name =>
      self
        ...
        .flatMap { case partitions =>
          scribe.info(f"Partitioning of $name done with ${partitions.size} partitions with sizes min=${partitions.map(_.numRows).min}%,2d max=${partitions.map(_.numRows).min}%,2d")
