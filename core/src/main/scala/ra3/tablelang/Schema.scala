...
object Schema {
  private[ra3] inline def fromIdent[T0<:Tuple](t:TableExpr.Ident{type T = ReturnValueTuple[T0]}) = {
  ...
  private[ra3] inline def fromDelayedIdent[A](e: Expr.DelayedIdent[A]) = {
    
    Schema[A*:EmptyTuple](List(e.asInstanceOf[Expr.DelayedIdent[Any]]))
  }
}
class Schema[A <: Tuple](
    private[ra3] val list: Seq[Expr.DelayedIdent[Any]]
) {
  ...
  val none: Expr[ReturnValueTuple[EmptyTuple]] = {
    ra3.lang.Expr
      .BuiltInOpAny(new ra3.lang.ops.OpAny.MkReturnValueTuple[EmptyTuple])(
        Nil
      )
  ...
  inline def columns: SchemaCurryT[A] = {

    val tup = scala.Tuple
      .fromArray(list.toArray)
      .asInstanceOf[Tuple.Map[A, ra3.lang.DelayedIdent]]
