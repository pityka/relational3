...
class StringBufferSuite extends munit.FunSuite with WithTempTaskSystem {
  test("toSegment") {
    withTempTaskSystem { implicit tsc =>
      val s : Seq[CharSequence] = Seq("0", "1", "2", "3", s"${Char.MinValue}", ".","")
  ...
  test("findInequalityVsHead") {
    val b1 = BufferString(Array[CharSequence]("0", "1", "2", "3", "4", "4", "5", "6"))
  ...
  test("groups") {
    val b1 = BufferString(Array[CharSequence]("0", "1", "2", "3", "4", "4", "5", "6").reverse)
  ...
  test("take") {
    val b1 = BufferString(Array("99", "1", "2", BufferString.missing, "4", "4", "5", "6"))
    assertEquals(
      b1.take(BufferInt(-1, 0, 3, 0, -1)).toSeq,
      Seq(BufferString.missing, "99", BufferString.missing, "99", BufferString.missing)
    )
  }
  test("positiveLocations") {
    val b1 = BufferString(Array(BufferString.missing, "1", "2", "", "4", "4", "5", "6"))
    assertEquals(b1.positiveLocations.toSeq, Seq(1, 2, 4, 5, 6, 7))
  }
  test("outer join") {
    val (a, b) = BufferString("0", BufferString.missing.toString, "1", "-99").computeJoinIndexes(
      BufferString("0", "0", BufferString.missing.toString, BufferString.missing.toString, "1", "1", "99"),
      "outer"
    )
  ...
  test("mergeNonMissing") {
    assertEquals(
      BufferString("0", BufferString.missing.toString, "0", "1",  BufferString.missing.toString)
        .mergeNonMissing(BufferString("1",  BufferString.missing.toString, "1", "0", "1"))
        .toSeq,
