...
class ConstantIntBufferSuite extends munit.FunSuite with WithTempTaskSystem {
  test("makeStatistic") {
    val st = BufferInt.constant(0,6).asInstanceOf[BufferIntConstant].makeStatistic()
  ...
  test("toSegment") {
    withTempTaskSystem { implicit tsc =>
      ...
      val segment = ColumnTag.I32
        .toSegment(BufferInt.constant(0,6), LogicalPath("toSegmentTest", None, 0, 0))
        .unsafeRunSync()
  ...
  test("toSegment empty") {
    withTempTaskSystem { implicit tsc =>
      val segment = ColumnTag.I32
        .toSegment(BufferInt.constant(0,0), LogicalPath("toSegmentTest2", None, 0, 0))
        .unsafeRunSync()
  ...
  test("positiveLocations") {
    ...
    assertEquals(BufferInt.constant(1,4).positiveLocations.toSeq, Seq(0,1,2,3))
  }
  test("outer join") {
